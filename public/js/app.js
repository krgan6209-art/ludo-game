...
async function joinRoom() {
  try {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      throw new Error('ارتباط با سرور هنوز آماده نیست؛ چند ثانیه صبر کنید و دوباره تلاش کنید');
    }
...
const BOARD_MAP = Array(225).fill('empty');

// مسیر قرمز و آبی
const redPath = [
  128,113,98,83,68,53,38,23,8,9,10,25,40,55,70,85,100,115,
  116,117,118,119,120,105,90,75,60,45,30,15,0,1,2,3,4,5,
  20,35,50,65,80,95,110,125,126,127
];

const bluePath = [
  8,9,10,25,40,55,70,85,100,115,116,117,118,119,120,105,
  90,75,60,45,30,15,0,1,2,3,4,5,20,35,50,65,80,95,110,
  125,126,127,128,113,98,83,68,53,38,23
];
...
bluePath.forEach((idx, i) => {
  if (i < 40) {
    BOARD_MAP[idx] =
      BOARD_MAP[idx] === 'path-red' ? 'safe' : 'path-blue';
  } else if (i < 46) {
    BOARD_MAP[idx] = 'home-blue';
  }
});

// خانه‌های امن
[8, 68, 128, 0, 120, 60].forEach(idx => {
  BOARD_MAP[idx] = 'safe';
});
...
// چهار خانه‌ی رنگی مثل لودوی واقعی
const decorativeHomes = {
  green: [15,16,17,18,30,31,32,33,45,46,47,48,60,61,62,63],
  yellow: [161,162,163,164,176,177,178,179,191,192,193,194,206,207,208,209]
};

Object.entries(decorativeHomes).forEach(([color, cells]) => {
  cells.forEach(idx => {
    if (BOARD_MAP[idx] === 'empty') {
      BOARD_MAP[idx] = `home-${color}`;
    }
  });
});

// محل ۴ مهره‌ی قرمز و آبی
const startCells = {
  red: [137, 138, 151, 152],
  blue: [21, 22, 36, 37]
};
...
function renderBoard() {
  ...
  [1, 2].forEach(pi => {
    ...
    const color = player.color || (pi === 1 ? 'red' : 'blue');
    const pieces = player.pieces || [];

    pieces.forEach((pos, idx) => {

      // مهره داخل خانه‌ی خودش
      if (pos === -1) {
        const cells = startCells[color] || [];
        const startIndex = cells[idx];

        if (startIndex !== undefined) {
          const cell = el('ludo-board').children[startIndex];

          if (cell) {
            cell.appendChild(
              createPiece(pi, idx, color)
            );
          }
        }
...
function createPiece(playerIndex, pieceIndex, color) {
  ...
  piece.onclick = () => {
    onPieceClick(playerIndex, pieceIndex);
  };
...
function onPieceClick(playerIndex, pieceIndex) {
  ...
  if (myPlayerIndex !== currentGame.state.currentTurn) {
    return;
  }
...
function updateTurn() {
  ...
  if (currentGame.state.currentTurn === myPlayerIndex) {
    status.textContent = '🎲 نوبت شماست';
    el('roll-btn').disabled = false;
  } else {
    status.textContent = '⏳ نوبت حریف';
    el('roll-btn').disabled = true;
  }

  const p1 = currentGame.state.players?.[1];
  const p2 = currentGame.state.players?.[2];

  if (el('player1-badge')) {
    el('player1-badge').textContent =
      p1?.id === user.id
        ? '🔴 شما • قرمز'
        : '🔴 بازیکن ۱ • قرمز';
  }
...
function startGame() {
  ...
  addSystemChat('🎮 بازی آماده است — هر بازیکن ۴ مهره دارد.');
}
